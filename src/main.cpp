      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Main car's localization Data
            ...
            if(too_close) {
              ref_vel -= 0.224;
              int lane_shift_val = 0;
              if (ref_vel < 40)
              {
                lane_shift_val = getLaneShiftVal(prev_size, lane, car_s, sensor_fusion);
                if (lane_shift_val > 0)
                {
                  std::cout << "curr_lane: "+std::to_string(lane) << std::endl;
                  std::cout << "lane_chane_val: "+std::to_string(lane_shift_val) << std::endl;
                }
                lane += lane_shift_val;
              }
            }
            ...
            else{ //  starting reference = previous path's end point
            ...
            for(int loc = 0; loc < ptsx.size(); loc++){
              // shift car reference angle to 0 degrees
              double shift_x = ptsx[loc] - ref_x;
              double shift_y = ptsy[loc] - ref_y;

              ptsx[loc] = (shift_x * cos(0-ref_yaw) - shift_y * sin(0-ref_yaw));
              ptsy[loc] = (shift_x * sin(0-ref_yaw) + shift_y * cos(0-ref_yaw));
            ...
            // Actual (x,y) points for the planner
            vector<double> next_x_vals;
            vector<double> next_y_vals;

            for (int loc = 0; loc < previous_path_x.size(); loc++){
              next_x_vals.push_back(previous_path_x[loc]);
              next_y_vals.push_back(previous_path_y[loc]);
            }

            // Break up spline points so that the desired reference velocity is achieved  
            ...
            for( int loc = 1; loc <= 50-previous_path_x.size(); loc++ ){
